...
import mothersDayPromo from '@/assets/mothers-day-promo1.png';
...
const MothersDayPopup = () => {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => {
      ...
      <DialogContent className="max-w-[90vw] sm:max-w-[500px] p-0 overflow-hidden border-none bg-transparent shadow-none z-[100] outline-none">
        <div className="relative group flex flex-col items-center">
          ...
          >
            ...
            {/* Overlay Reserve button placed on top of the image */}
            <div className="absolute left-1/2 -translate-x-1/2 bottom-20 w-[80%] sm:w-2/3 px-6 pb-4">
              <Button
                onClick={handleReserve}
                style={{ backgroundColor: '#d9a835', color: '#0a0a0a' }}
                className="w-full font-bold pb-3 pt-6 sm:pt-3 text-lg rounded-xl shadow-[0_6px_20px_rgba(0,0,0,0.25)] transition-transform hover:scale-[1.02] active:scale-[0.98] flex items-center justify-center gap-2"
              >
          ...
          {/* Date & Hours boxes (below the image) */}
          <motion.div
            initial={{ y: 10, opacity: 0 }}
            animate={{ y: 0, opacity: 1 }}
            transition={{ delay: 0.9 }}
            className="w-full px-6 mt-3"
          >
